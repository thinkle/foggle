<script>
    ...
    let feedbackPerGuess = $derived(
        guesses.map(
            (g,i) => (
                computeFeedback(guesses.slice(0,i+1), theWord)
            )
        )
    );
...
<div>
    {#snippet template(word, expanded)}
        ...
        {#if letters > 1}
            ...
            {/if}
            <Word word={word} answer={null}/>
            and    
        {/if}
    {/snippet}
    <SpeechBubble position="right">
        The word was:
        <Word word={theWord} answer={theWord} feedback={Array.from(theWord).map((l)=>CORRECT_B)} />
        Let's break down how we got there...
    </SpeechBubble>
    {#each guesses as guess, i}
        {@const nwords = possibleWordsPerGuess[i].length}
        <SpeechBubble position={i % 2 == 0 ? 'right' : 'left'} 
            ...
            {:else}
                <div class="blurb">
                    ...
                    {/if}              
                    guess was:
                    <Word word={guesses[i]} answer={theWord} 
                        feedback={feedback.guessFeedback[i]}/>   
                    {#if feedbackPerGuess[i].template !== feedbackPerGuess[i-1]?.template || 
                        possibleWordsPerGuess[i].length !== possibleWordsPerGuess[i-1]?.length}                                            
                        {#if i==0}
                            <br/>After that guess, 
                            you were down to {@render template(feedbackPerGuess[i].template,true)}
                        ...
                        {:else if i < (guesses.length - 1)}
                            ...
                            <br/><span>{possibleWordsPerGuess[i].length}
                ...
                {#if possibleWordsPerGuess[i].length !== possibleWordsPerGuess[i-1]?.length}
                    {#if nwords > 1 && nwords < 5} 
                        {#each possibleWordsPerGuess[i] as word, j}
                            <span class='word'>{word}</span>{#if j == nwords - 2}&nbsp;and&nbsp;{:else if j < nwords - 1}, {/if}
...
<style>
    ...
    span.word {
        font-variation-settings: 'EMPH' 100, 'SLANT' 0;
        font-style: normal;
    }
    button {
        background: none;
